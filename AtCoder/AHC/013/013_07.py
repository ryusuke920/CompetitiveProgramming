                    ...
                    if 0 <= y < n:
                        if grid[y][j] == max_num and grid[i][j] == 0:
                            ...
                            if dy == 2:
                                if grid[y - 1][j] != 0:
                                    check = False
                            if num + (abs(y - i)) <= 100 * k and check:
                                move(y, j, i, j)
...
def move(prev_y: int, prev_x: int, next_y: int, next_x: int) -> None:
    ...
    if prev_y == next_y:
        if prev_x < next_x:
            for i in range(prev_x, next_x):
                move_list.append((prev_y, i, next_y, i + 1))
                num += 1
        if prev_x > next_x:
            for i in reversed(range(next_x, prev_x)):
                move_list.append((prev_y, i + 1, next_y, i))
                num += 1
    if prev_x == next_x:
        if prev_y < next_y:
            for i in range(prev_y, next_y):
                move_list.append((i, prev_x, i + 1, next_x))
                num += 1
        if prev_y > next_y:
            for i in reversed(range(next_y, prev_y)):
                move_list.append((i + 1, prev_x, i, next_x))
                num += 1
...
def search() -> None:
    ...
    for cluster in range(k):
        ...
        for j in range(l - 1):
            ...
            if is_ok and obstacle and num != 100 * k:
                connect(yi, xi, yj, xj)
            ...
            if xi == xj and abs(yi - yj) <= 10:
                ...
                for dy in range(yi, yj):
                    ...
                    if dy != yi and grid[dy][xi]:
                        obstacle = False
                    if dy != yi:
                        yoko &= (w_line[dy][xi - 1] and w_line[dy][xi])
                is_ok &= yoko
            else:
                is_ok = False
